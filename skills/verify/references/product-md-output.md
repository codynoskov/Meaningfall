# ProductMD Output Verification

Per-owner checks applied at the output gate before accepting a generated or
updated `PRODUCT.md`. When the output was produced through a Seed Loop process,
this is the domain-specific `settled` check. These are the applied checks; the
authority for what is valid is the ProductMD spec. When in doubt, the spec wins.

Load this when finishing an owner or before final output. Each owner lists what
belongs, the sharpest "looks-like-but-isn't" distinctions, and shape checks.
