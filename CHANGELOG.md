...
- Consolidates changelog, contributing, and notice metadata at the repository root.
  Package folders no longer carry package-local `CHANGELOG.md`, `CONTRIBUTING.md`,
  or `NOTICE` files by default.
- Adds an Action Modes section to root `SPEC.md` defining shared workflow
  vocabulary — `explicit`, `automatic`, `suggested` — as a per-action property
  orthogonal to enablement. Durable memory or file transitions default to
  `suggested`; the vocabulary grants no automatic behavior by itself; and
  irreversible, destructive, security-sensitive, or history-rewriting actions
  require explicit confirmation even when requested.
- Applies per-workflow Action Mode grants (decision 0072): Guide, Install, Verify,
  Memory, Seed Loop, and Git Hygiene each declare which concrete actions are
  `explicit`, `automatic`, or `suggested` in a short Action Modes section in their
  own contract. Grants are per workflow and per action, never global.

### ProductMD

- Updates README and contribution boundaries to point guided creation at the canonical
  root `skills/guide/` public skill.
- Records that ProductMD output verification moved from Product Brief references into
  the neutral root `skills/verify/` public skill. Guide now consumes Verify target
  `product-md-output` instead of owning the applied output gate.

### Memory

- Adds a suggestion-only discipline for surfacing repeated durable human
  instructions: when the human repeats a project-level instruction, correction,
  preference, or workflow across more than one task in a way that looks durable,
  the agent offers to record it, asks first, proposes destination and wording, and
  never records silently. Adds no installed files, commands, or state, and does not
  activate Reconcile or Impact.
- Adds the Memory surface classes (Product Definition, Guidance, Memory Index,
  Memory Records, Cold Archive) and the Memory Records format. Installed supporting
  memory moves from a file-first tree to `notes.md`, `drafts/`, `decisions/`,
  `glossary.md`, and a lazy `archive/`; folder-backed `drafts/` and `decisions/`
  use YAML front matter with closed `status` enums; `notes.md` and `glossary.md`
  stay simple Markdown. `facts.md` and `learnings.md` are no longer default
  surfaces and route by role; a reusable lesson is a decision with `type: learning`.
- Adds an Archive Lifecycle for the Cold Archive surface (decision 0074): lazy
  `archive/`; deprecated or superseded decisions stay active records until worth
  archiving as artifacts; archive is not Git history; archive moves are suggested.
  Memory stays usable without Git; the accepted installed-user Git scope is owned by
  the new Git Hygiene skill.
- Adds an advisory "what next" planning readout as a Memory read capability
  (decision 0073): the agent answers what-next from `PRODUCT.md`, notes, drafts,
  decisions, and implementation state. Advisory only — no tasks, no roadmap, no
  memory transition, and no separate skill or command.

### Install

- Delivers the repeated-material capture discipline as local operating text and
  moves the recommended `AGENTS.md` product-memory snippet from
  `meaningfall:product-memory v1` to `v2`; updates replace a found v1 block with
  v2. Install-contract generation v1.2.
- Updates the `product-memory` install shape to the accepted records format:
  `notes.md`, `drafts/`, `decisions/`, `glossary.md`, and lazy `archive/`, with no
  default `facts.md` or `learnings.md`. Folder-backed records carry YAML front
  matter; the generated `README.md`, routing, and install gate are updated to
  match.

### Verify

- Adds target `repository-surface` for low-noise public repository surface checks
  against root `SPEC.md`, root `README.md`, and `index.yaml`. This is a structural
  gate, not a broad documentation audit, and installs no `index.yaml` or
  `state.yaml` into user workspaces.
- Adds target `product-memory` for installed product-memory checks: no
  `product-memory/state.yaml`; required YAML front matter and closed `status` enums
  for folder-backed `drafts/` and `decisions/` records; no `facts.md`/`learnings.md`
  default surface; and no captured operating instruction inside the marked
  Meaningfall `AGENTS.md` block. It reports findings and does not silently fix
  files.

### Git Hygiene

- Adds Git Hygiene as a governed public root skill (`skills/git-hygiene/`) and
  accepts installed-user Git behavior under Action Modes (decision 0071). It is
  conservative by default: read-only Git detection, `suggested`/`explicit`
  checkpoints, exact-path staging, no silent commits, no hooks, and no `git init`
  by default. An opt-in auto-checkpoint is available but default-off, and
  destructive or history-rewriting actions require explicit confirmation.

## ProductMD v0.1.12 - 2026-06-19
...
## Seed Loop v0.1.0 - 2026-06-13

Initial Seed Loop package:

- Established Seed Loop as the neutral grow/root/settled discipline for dependency-ordered artifacts.
- Defined the node vocabulary: seed, joint, and leaf.
- Defined grow and root passes, the settled gate, locality, and the mechanism/oracle separation.
- Kept Seed Loop domain-neutral: consumers reference it, and it references no consumer.
