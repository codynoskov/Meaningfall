...
This module spec follows the repository-level contract in [`../../SPEC.md`](../../SPEC.md). It inherits the root public-surface and contract rules, and narrows or extends them only where this spec says so explicitly.
...
Per root `SPEC.md` Action Modes and decision 0072, a workflow applying the Seed
Loop grants:

- `automatic`: reading nodes and rooting them in dependency order (observe-only
  passes).
- `suggested`: proposing repairs or contract, routing, and context updates to
  dependent nodes after a change.
- `explicit`: applying those repairs across separately owned artifacts, unless an
  accepted workflow already covers them.

This is a grant for workflows that apply the Seed Loop; it does not make the Seed
Loop reference any specific consumer.
